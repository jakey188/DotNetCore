    ...
    public class DependencyPack
    {
        ...
        protected virtual void AddToServices(IServiceCollection services, Type implementationType)
        {
            ...
            for (int i = 0; i < serviceTypes.Length; i++)
            {
                ...
                bool multiple = serviceType.HasAttribute<DependencyMultipleAttribute>();
                if (i == 0)
                {
                    if (multiple)
                    {
        ...
        protected virtual Type[] GetImplementedInterfaces(Type type)
        {
            ...
            Type[] interfaceTypes = type.GetInterfaces().Where(t => !exceptInterfaces.Contains(t) && !t.HasAttribute<DependencyIgnoreAttribute>()).ToArray();
        ...
        protected virtual Type[] FindDependencyTypes()
        {
            ...
            Type[] baseTypes = { typeof(ISingletonDependency), typeof(IScopedDependency), typeof(ITransientDependency) };
            return types.Where(type => type.IsClass && !type.IsAbstract && !type.IsInterface
                && !type.HasAttribute<DependencyIgnoreAttribute>()
